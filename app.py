...
function apiBase(){let source='';if(window.location.ancestorOrigins&&window.location.ancestorOrigins.length)source=window.location.ancestorOrigins[0];if(!source&&document.referrer)source=document.referrer;let base;try{base=new URL(source)}catch(e){base=new URL('http://localhost:8501')};if(base.hostname.endsWith('.app.github.dev'))base.hostname=base.hostname.replace(/-8501(?=\.app\.github\.dev$)/,'-8000');else{base.hostname='localhost';base.port='8000'}return base.origin}
...
button.onclick=async()=>{const file=fileInput.files[0];if(!file){status.textContent='Choose an MP4 or MOV file first.';status.className='error';return}button.disabled=true;bar.style.width='0%';status.className='';const api=apiBase();try{status.textContent='Connecting to '+api+'...';await jsonRequest(api+'/health',{method:'GET',credentials:'include'});const start=await jsonRequest(api+'/uploads/start',{method:'POST',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify({filename:file.name,size:file.size})});const chunks=Math.ceil(file.size/start.chunk_size);for(let i=0;i<chunks;i++){const chunk=file.slice(i*start.chunk_size,Math.min(file.size,(i+1)*start.chunk_size));await jsonRequest(api+'/uploads/'+start.upload_id+'/chunks/'+i,{method:'POST',credentials:'include',headers:{'Content-Type':'application/octet-stream'},body:chunk});const percent=Math.round(((i+1)/chunks)*100);bar.style.width=percent+'%';status.textContent='Uploading... '+percent+'%'}await jsonRequest(api+'/uploads/'+start.upload_id+'/complete',{method:'POST',credentials:'include'});status.textContent='Upload complete. Click “Use uploaded video” below.';status.className='success'}catch(error){status.textContent='Upload failed: '+error.message+' · Endpoint: '+api;status.className='error'}finally{button.disabled=false}};
</script></body></html>
"""
